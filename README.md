...
| **Adding to losers** | Only when risk is already tiny (small lots, low %). Stops on every leg go **wider** (default 2×) so the first trade is not stopped out. Otherwise the extra is closed. |
...
Open **Inputs**. Six groups, 31 settings, plain language. Most people only change:
...
| [docs/CHANGELOG.md](docs/CHANGELOG.md) | What changed in 1.30 / 1.29 / 1.28 / … |
